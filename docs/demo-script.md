...
4. Start `npm run watch:excel` in the terminal if it is not already running.
5. Save the workbook.
6. Point out that the watcher commits and pushes only the workbook.
7. Open GitHub Actions and show `.github/workflows/crm-export-to-main.yml` running.
8. When it completes, refresh GitHub Pages and point out that the numbers changed first, then the AI copy changed to match.
9. Open `site/data/pipeline.json` and `site/data/ai-summary.json` to show the separation between facts and AI-authored text.
